...
```{r ex1}
  lm(Ozone ~ Solar.R + Wind + Temp,data = airquality) -> firstreg
```
...
```{r}
lm(Ozone ~ Wind, data = airquality) -> charles
summary(charles)
plot(charles)
coef(charles)
```
...
```{r}
plotreg +
  geom_segment(aes(x = Temp, xend = Temp, y = Ozone, yend = .fitted), alpha = 0.2)
```
...
df %>% 
  ...
  lm(lifeExp ~ gdpPercap, data = .) %>% 
  tidy()
# ne marche pas , summarise gère des fonctions qui retournennt une seule valeur
df %>% 
  group_by(year, continent) %>% 
...
```{r}
objetcorr <- cor.test(df$lifeExp, df$gdpPercap)
```
