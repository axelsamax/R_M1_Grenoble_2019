...
```{r ex2}
#1. diire à gather quelles variables recueillir
table4b %>% 
...
#2. dire à gather ce qu'on ne veut pas recueillir
table1 %>% 
...
wbp %>% 
  gather(-country, -indicator, key = year, count = values)
```
...
```{r ex5}
table2 %>% 
  spread(key = type, value = count) -> table2_postexo
...
df %>% 
  ...
  group_by(decennie, sex) %>% 
  filter(n == max(n)) %>% 
  select(decennie, sex, name) -> temp
temp %>% 
